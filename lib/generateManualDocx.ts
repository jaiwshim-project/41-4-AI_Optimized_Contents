    ...
    sections: [
      {
        ...
        children: [
          ...
          // 6. A/B 버전 비교 & SNS 변환
          h2('6. A/B 버전 비교 & SNS 변환'),
          h3('A/B 버전 비교'),
          body('콘텐츠 생성 시 "A/B 버전 생성" 토글을 켜면 전문적/친근한/설득적 3가지 톤으로 동시 생성됩니다.'),
          bullet('결과 페이지 상단 탭을 클릭하여 각 버전 전환 비교'),
          bullet('가장 적합한 버전을 선택하여 복사, 편집, 이미지 생성 등 활용'),
          spacer(),
          h3('SNS 채널별 변환'),
          body('결과 페이지 하단의 SNS 변환 섹션에서 원하는 채널 버튼을 클릭하면 자동 변환됩니다.'),
          bullet('인스타그램: 후킹 문구 + 핵심 포인트 + 이모지 + 해시태그'),
          bullet('링크드인: 전문적 인사이트 + 번호 정리 + 토론 유도'),
          bullet('네이버 블로그: SEO 제목 + 목차 + 키워드 최적화'),
          bullet('카드뉴스: 6-8장 슬라이드 구성 (표지/본문/마무리)'),
          bullet('핵심 요약: 3줄 요약 + 키워드 5개 + 한 줄 결론'),
          spacer(),
          ...
          // 15. A/B 버전 생성
          h2('15. A/B 버전 생성'),
          body('같은 주제로 3가지 다른 톤의 콘텐츠를 동시에 생성하여 최적의 버전을 선택합니다.'),
          spacer(),
          h3('사용 방법'),
          ...
          spacer(),
          h3('사용 방법'),
          bullet('1. 콘텐츠 생성 후 결과 페이지 하단의 "SNS 채널별 변환" 섹션 확인'),
          bullet('2. 원하는 채널 버튼 클릭'),
          bullet('3. AI가 해당 채널에 최적화된 형식으로 자동 변환'),
          bullet('4. "복사" 버튼으로 클립보드에 복사'),
          spacer(),

          // 17. 대시보드 통계
          h2('17. 대시보드 통계 & 차트'),
          body('콘텐츠 생성/분석 현황을 차트와 통계로 한눈에 파악합니다.'),
          spacer(),
          h3('통계 항목'),
